...
class WikiProjectAssess:
    ...
    def qualitylist(self):
        for wikiproject, category in self.unknownquality.items():
            ...
            to_process = qualitypredictor(to_process)
            ...
            for pair in to_process:
                article = pair[0]
                prediction = pair[1]
                contents += "<b>[[" + article + "]]</b> ([[Talk:" + article + "|talk]])<br />Predicted quality: " + prediction + "|"
            contents = contents[:-1] + "}}"
    ...
    def scopepredictor(self):
        # This query produces a list of pages that belong to categories that have been tagged by the WikiProject
