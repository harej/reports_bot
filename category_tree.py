...
def build_cat_tree(cat_name, max_depth=5):
    ...
    query = wptools.query('wiki', 'select distinct page.page_title from categorylinks join page on categorylinks.cl_from=page.page_id where page_namespace = 14 and cl_to = %s and page_title like "%\_WikiProjects";', cat_name)
    retval = {}
    for row in query:
