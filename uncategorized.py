...
def main(rootpage, saveto):
    ...
    for t in contents:
        if t.name.strip() == "WikiProject directory entry small":
            ...
            query = wptools.query('wiki', 'select distinct cl_to from categorylinks join page on categorylinks.cl_from=page.page_id where page_namespace in (4, 14) and page_title = "{0}" and cl_to in %s;'.format(project), whitelist)
